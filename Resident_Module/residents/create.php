    ...
    if (empty($about) || empty($post) || empty($location) || empty($date_from) ||
        ...
        if ($connection->query($sql) === false) {
            ...
            if ($connection->query($sql) === false) {
                ...
                $sql = "SELECT * FROM resident_account AS ra INNER JOIN users ON ra.users_id = users.id where ra.id=$resident_id";
...
<div class="d-flex align-self-center justify-content-center">
        ...
        <div class="row mb-3">
            ...
            <div class="row mb-3">
    <label class="col-sm-6 col-form-label">Resident:</label>
    <div class="col-sm-6">
        <select class="custom-select" id="resident_id" name="resident_id" style="width:150px;">
            <option value="">Select Resident</option>
            <?php
            // Fetch the resident ID and name data from the database
            $sql = "SELECT id, Name FROM resident_account";
            $result = mysqli_query($connection, $sql);

            // Loop through the results and create the select options
            while ($row = mysqli_fetch_assoc($result)) {
                $resident_id = $row['id'];
                $resident_name = $row['Name'];
                echo "<option value='$resident_id'>$resident_name</option>";
            }
            ?>
        </select>
    </div>
